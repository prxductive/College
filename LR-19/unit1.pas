...
var
  InputValue: Double;
begin
  if Edit1.Text <> '' then  // Проверяем, не пуста ли строка
  begin
    try
      InputValue := StrToFloat(Edit1.Text);
      if InputValue < 0 then  // Проверяем, является ли число отрицательным
      begin
        Edit1.Text := '0'; // Выводим 0 для отрицательного числа
        LastInputIsOperation := False;
      end
      else
      begin
      ...
      on E: Exception do
      begin
        Edit1.Text := '0'; // В случае ошибки выводим 0
        LastInputIsOperation := False; // Сбрасываем флаг после ошибки
      end;
    end;
  end
  else
  begin
    Edit1.Text := '0'; // В случае пустой строки выводим 0
    LastInputIsOperation := False; // Сбрасываем флаг после ошибки
  end;
end;
